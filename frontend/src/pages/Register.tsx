  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#07080A] text-[#F0F4F2] font-sans selection:bg-[#3ECF8E]/30">
      ...
      <div className="relative flex items-stretch min-h-screen flex-col lg:flex-row">
        ...
        <section className="flex-[1.2] flex flex-col justify-center gap-[clamp(24px,3.4vh,42px)] p-[clamp(34px,4.6vw,78px)_clamp(26px,4.4vw,62px)] hidden md:flex">
          ...
          <div className="relative w-full max-w-[416px] rounded-[26px] p-[clamp(26px,2.6vw,36px)] bg-[#16191A]/70 backdrop-blur-[34px] border border-white/10 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.1),inset_0_-1px_0_0_rgba(0,0,0,0.45),0_46px_100px_-46px_rgba(0,0,0,0.95)]" style={{ animation: 'v2-rise 760ms cubic-bezier(.16,.9,.2,1) both' }}>
            ...
            <div className="flex flex-col gap-[26px]">
              <div className="flex flex-col gap-[7px]" style={{ animation: 'v2-rise 640ms cubic-bezier(.2,.75,.3,1) 120ms both' }}>
                <h2 className="m-0 font-semibold text-[27px] leading-[1.12] tracking-[-0.028em]">Create your account</h2>
                <p className="m-0 font-light text-[13.5px] leading-[1.5] text-[#F0F4F2]/50">Join Wrench in under a minute</p>
              </div>
              ...
              <form onSubmit={handleRegister} className="flex flex-col gap-[16px]">
                ...
                <label className="flex flex-col gap-[8px]" style={{ animation: 'v2-rise 640ms cubic-bezier(.2,.75,.3,1) 250ms both' }}>
                  <span className="font-mono font-medium text-[9px] leading-none tracking-[0.2em] transition-colors duration-300" style={{ color: focusField === 'phone_number' ? '#3ECF8E' : 'rgba(240,244,242,0.45)' }}>PHONE</span>
                  <div className="relative group">
                    <input type="tel" placeholder="+1234567890" autoComplete="tel" required
                      name="phone_number"
                      value={formData.phone_number} onChange={handleChange}
                      pattern="\+?[1-9]\d{1,14}" title="International format, e.g. +11234567890"
                      onFocus={() => setFocusField('phone_number')} onBlur={() => setFocusField(null)}
                      className="w-full h-[51px] px-[15px] rounded-[14px] bg-white/5 border border-white/10 text-[#F0F4F2] font-light text-[15px] leading-none outline-none transition-all duration-300 hover:border-white/20 focus:border-[#3ECF8E]/60 focus:shadow-[0_0_0_3px_rgba(62,207,142,0.14)] focus:bg-white/10"
                    />
                    <div className="absolute left-[14px] right-[14px] bottom-0 h-px overflow-hidden pointer-events-none transition-opacity duration-300" style={{ opacity: focusField === 'phone_number' ? 1 : 0 }}>
                      <div className="w-[32%] h-full" style={{ background: 'linear-gradient(90deg, transparent, #3ECF8E, transparent)', animation: 'v2-sweep 1500ms cubic-bezier(.45,0,.55,1) infinite' }}></div>
                    </div>
                  </div>
                </label>
                ...
                {error && (
                  <div role="alert" className="flex items-center gap-[10px] p-[11px_13px] rounded-[12px] bg-[#E07864]/10 border border-[#E07864]/25 mt-2" style={{ animation: 'v2-rise 260ms ease both' }}>
